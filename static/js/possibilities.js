...
function loadPossibilities() {
	$.get('/possibilities.json', function(data){
		...
		var tr = $('<tr></tr>');
		tr.append($('<td></td><td>Phrase</td><td>Date Added</td>'));
		$(head).append(tr);
		$(table).append(head);
		...
		data.forEach(function(poss){
			tr = $('<tr></tr>');
			var td = $('<td>' + num + '</td><td>' + poss.square + '</td><td>' + poss.dateAdded + '</td>');
			$(tr).append(td);
			$(body).append(tr);		
			num++;
		});
		...
		form.append($('<input class="hidden" value="1" name = "numDrinks" type="text" id="numDrinks" placeholder="Number of Drinks"></br>'));
